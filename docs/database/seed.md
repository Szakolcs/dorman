...
| Active period | ~18 months of history | Drives time-series volumes |
| Staff users | ~25 | Admins, managers, reception, maintenance, security |
...
| users | 425 | 25 staff + 400 tenant-linked accounts; mix of principal_type |

| Table | Rows | Notes |
| --- | --- | --- |
| flats | 90 | ~30 per building, distributed across floors 0–5 |
| rooms | 300 | Avg 3.3 rooms/flat, capacity 1–3, ~5% archived |

| Table | Rows | Notes |
| --- | --- | --- |
| tenants | 400 | 80% linked to a user; degree (BSc/MSc/PhD), faculty, age 18–30 |
| user_roles | 450 | One per staff/tenant + ~25 multi-role staff |
| chat_tenant_profiles | 400 | 1:1 with tenants (unique constraint) |
| room_assignments | 520 | ~400 active + ~120 historical move-outs |
...
| Table | Rows | Notes |
| --- | --- | --- |
| inventory_items | 800 | ~600 in rooms/flats, ~200 building-level common assets |
| item_loans | 500 | ~150 currently checked out, rest returned |
| maintenance_tickets | 600 | Mix of statuses; ~70% closed |
| ticket_status_changes | 1,500 | Avg 2.5 transitions per ticket |
| operational_jobs | 300 | Cleaning/inspection/maintenance assignments |
| packages | 1,000 | ~85% picked up |
| package_notifications | 1,500 | Avg 1.5 per package across email/sms/push |

| Table | Rows | Notes |
| --- | --- | --- |
| tenant_entry_tokens | 500 | Avg 1.25 per tenant (some renewed) |
| access_events | 8,000 | ~20 entries/exits per tenant; weighted by daytime |
| guest_visits | 800 | ~2 per active tenant over the period |
| guest_access_events | 1,600 | Check-in + check-out per visit |

| Table | Rows | Notes |
| --- | --- | --- |
| chat_rooms | 490 | 90 flat rooms (1:1 with flat) + 400 direct DM pairs |
| chat_room_members | 2,400 | Flat: ~4 members each; DMs: 2 each; some historical |
| chat_messages | 15,000 | Avg ~30 per room, skewed (some rooms much busier) |
...
| forum_poll_votes | 2,500 | ~25 votes per poll on average |
| forum_attendance_intents | 1,500 | Going/maybe/no for event posts |
| forum_comments | 3,000 | Avg ~4 per post; some threaded replies |
| forum_reactions | 5,000 | Likes etc. across posts/comments |
| forum_post_views | 20,000 | Many views per published post |
| forum_moderation_actions | 100 | Rare moderation events |

| Table | Rows | Notes |
| --- | --- | --- |
| audit_events | 10,000 | Broad coverage of admin/system actions over 18 mo |
